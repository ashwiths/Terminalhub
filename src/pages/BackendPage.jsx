        ...
        <main className="min-h-screen relative">
            ...
            <section className="max-w-7xl mx-auto px-6 pt-14 pb-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
                    <div>
                        <p className="text-emerald-400 font-mono text-xs font-bold tracking-widest uppercase mb-3">
                            Backend Guide
                        </p>
                        ...
                        <p className="text-slate-400 text-lg max-w-2xl">
                            The most popular Node.js frameworks for REST APIs, GraphQL, and Full-stack architectures.
                        </p>
                    </div>
                    <div className="flex items-center gap-2 bg-surface-700 border border-surface-500 rounded-xl px-5 py-3 self-start md:self-auto">
                        <span className="font-mono text-emerald-400 text-2xl font-black">{backendFrameworks.length}</span>
            ...
            <section className="max-w-7xl mx-auto px-6 pb-8">
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input
                            type="text"
                            placeholder="Search backend frameworks…"
                            value={search}
                         ...
                         focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20 transition-all"
                        />
                        {search && (
                            <button
                                onClick={() => setSearch('')}
            ...
            <section className="max-w-7xl mx-auto px-6 pb-24">
                <p className="text-xs text-slate-600 font-mono mb-5">
                    Showing <span className="text-emerald-400">{filtered.length}</span> of {backendFrameworks.length} Node.js frameworks
                ...
                {filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
                        ...
                        <p className="text-lg font-semibold text-slate-400">No backend frameworks found</p>
                        <p className="text-sm text-slate-600">Try a different keyword or clear the filters</p>
                        <button
                            onClick={() => { setSearch(''); setCategory('All'); }}
                            className="mt-2 px-5 py-2.5 rounded-xl border border-emerald-400/40 text-emerald-300
                         text-sm font-medium hover:bg-emerald-400/10 transition-all"
                        >
                            Clear filters
                        </button>
                    </div>
                    ...
                    <div className="columns-1 sm:columns-2 lg:columns-3 gap-5 space-y-5">
                        {filtered.map((fw, i) => (
                            <div key={fw.id} className="break-inside-avoid rounded-2xl">
                                <BackendCard fw={fw} index={i} onShowSteps={setSelectedFw} />
                            </div>
                        ))}
                    </div>
                )}
            ...
            {selectedFw && (
                ...
                >
                    <div
                     ...
                     shadow-[0_20px_60px_-15px_rgba(0,0,0,0.9),0_0_0_1px_rgba(52,211,153,0.15)]
                        ...
                        <div className="flex justify-between items-start gap-4 p-6 sm:p-8 border-b border-surface-500 bg-surface-900/60 flex-shrink-0">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm">Complete project creation workflow</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setSelectedFw(null)}
