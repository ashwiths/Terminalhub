...
export function StepRow({ number, step, os }) {
    const cmd = step[os];
    const isMultiLine = cmd && cmd.includes('\n');
    return (
        <div className="flex flex-col gap-1.5">
            <div className="flex items-start gap-2 mt-2">
                <span className="text-yellow-400 font-mono text-xs font-bold select-none whitespace-nowrap">
                    Step {number}
                </span>
...
export default function PythonCard({ fw, onShowSteps }) {
    const [copiedMain, setCopiedMain] = useState(false);
    const mainCmd = fw.steps?.[2]?.[`linux`] ?? ''; // Pip install command
        ...
        >
            ...
            <div className="flex items-start justify-between gap-3">
                ...
                <span
                    ...
                    style={{
                        color: fw.badgeColor === '#ffffff' ? '#e2e8f0' : fw.badgeColor,
                        borderColor: `${fw.badgeColor === '#ffffff' ? '#e2e8f0' : fw.badgeColor}44`,
                        background: `${fw.badgeColor === '#ffffff' ? '#e2e8f0' : fw.badgeColor}12`,
                    }}
                >
                    {fw.badge}
                </span>
            ...
            <div className={`flex ${mainCmd.includes('\n') ? 'items-start' : 'items-center'} gap-2 bg-surface-900 border border-surface-500 rounded-xl px-4 py-3`}>
                ...
                <button
                    ...
                    className={`flex-shrink-0 p-1.5 rounded-lg border transition-all duration-150 ${copiedMain
                        ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400'
                        : 'border-surface-500 text-slate-500 hover:border-yellow-400 hover:text-yellow-400'
                        }`}
                >
                    {copiedMain ? (
            ...
            <button
                ...
                className="flex items-center justify-between w-full px-4 py-2.5 rounded-xl border
                    border-surface-500 text-slate-400 hover:border-yellow-400/40 hover:text-yellow-400
